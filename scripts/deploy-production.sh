...
create_backup() {
    ...
    # Каталог уже создан с правами 700 — это второй рубеж, а не единственный.
    # Поэтому НЕ фатально: ронять полностью снятый и проверенный бэкап
    # из-за неудавшегося доп. ужесточения прав несоразмерно. Ровно так
    # второй боевой прогон потерял успешный бэкап и оставил писателей
    # остановленными.
    if ! docker run --rm -v "$BACKUP_DIR":/b alpine chmod -R go-rwx /b 2>/dev/null; then
        warning "Не удалось ужесточить права в $BACKUP_DIR — каталог и так 700"
    fi
...
update_code() {
    ...
    # `-20`, а НЕ `| head -20`. При выкатке длиннее 20 коммитов head
    # закрывает пайп, git получает SIGPIPE и завершается кодом 141;
    # под `set -Eeuo pipefail` это молча убивает скрипт ПРЯМО ЗДЕСЬ —
    # до `exec` ниже, то есть выкатка не делает вообще ничего, а журнал
    # обрывается на списке коммитов без единого слова об ошибке.
    # Ровно так провалилась выкатка 41 коммита 2026-08-05.
    # Поведение git-специфично: `seq 41 | head -20` в тех же условиях
    # возвращает 0, поэтому остальные конвейеры в файле не затронуты.
    git --no-pager log --oneline -20 "$before..$after"
