...
> **Исполнен на проде 2026-08-07.** Сервер 17.10 → **18.4**, том
> `app_postgres_data` → `app_postgres_data_pg18` (старый сохранён как точка
> отката). Счётчики до и после совпали: 3 / 16 / 554 / 802 / 950 / 46 / 16;
> `alembic heads` == `current` == `g1h2i3j4k5l6`; дрейф `alembic check`
> побайтово совпал с эталоном pg17; шесть режимов smoke — `PASS`.
> Простой ~12 минут: `active`/`reserved` Celery были пусты, дренажа не было.
>
> Составлен по результатам локальной репетиции 2026-08-04 на dev-стеке, но
> **перед боевым окном исправлен по шести расхождениям**, найденным сверкой
> с продом: монтирование `/docker-entrypoint-initdb.d` (блокер, §0 «Четвёртое»),
> набор расширений и критерий приёмки (§5, §6 — репетиция шла на dev, где
> расширения другие), невозможность пина версии `vector` (§5), метод сравнения
> `alembic check` (§6 — эталон и сверка снимались разными способами)
> и `GOOGLE_APPLICATION_CREDENTIALS` в smoke (§6).
...
> **Осторожно с `set -a; . ./.env`, если в той же оболочке потом идёт
> `docker compose up`.** Проверено ценой простоя прода 2026-08-07 (при
> добавлении IP в allowlist, не в этом окне): в `.env` значения
> `MONITOR_PASSWORD` и `MONITOR_PASSWORD_HASH` записаны с УДВОЕННЫМ `$`
> (`$$2a$14$…`) — так требует интерполяция compose. Оболочка же разворачивает
> `$$` в **PID процесса**, поэтому после `. ./.env` в окружении оказывается
> искалеченный хеш, и он ПЕРЕБИВАЕТ значение из `.env` при запуске compose.
> Caddy падает на провижининге: `base64-decoding password: illegal base64
> data`, контейнер уходит в restart-цикл, сайт отдаёт пустой ответ.
>
> Правило: `set -a; . ./.env` — только для `psql`/`pg_dump`-команд, где нужны
> `$DB_USER`/`$DB_PASSWORD`. Перед любым `docker compose up` эти переменные
> в оболочке иметь НЕЛЬЗЯ: compose сам читает `.env` и делает это правильно.
> Проверять так: `docker exec <контейнер> printenv MONITOR_PASSWORD_HASH | wc -c`
> — корректный bcrypt даёт 60 символов, значение из `.env` без обработки — 63.
...
**Обновление `vector` неизбежно — это не выбор.** На проде до переезда
SQL-версия расширения была `0.8.2`, хотя образ 17 нёс 0.8.6
(`ALTER EXTENSION … UPDATE` никто не делал). Первая редакция этого раздела
предлагала сохранить паритет через `CREATE EXTENSION vector VERSION '0.8.2'`
— **проверено в окне 2026-08-07: так нельзя.** Образ 18 несёт ровно один
установочный скрипт, своей текущей версии:

```
select version from pg_available_extension_versions where name='vector';
 0.8.6          -- и больше ничего

CREATE EXTENSION vector VERSION '0.8.2';
ERROR: extension "vector" has no installation script nor update path for version "0.8.2"
```

pgvector кладёт базовый скрипт `vector--<текущая>.sql` плюс апгрейд-пути
`vector--A--B.sql`; путей вниз нет. Поэтому на новом томе расширение
создаётся дефолтом образа — **0.8.6**, и это ожидаемое значение для §6.
Тип `vector` в дампе от SQL-версии расширения не зависит, восстановление
это не ломает (проверено: 225 TOC, счётчики совпали).
...
#
# ВАЖНО, проверено в окне 2026-08-07: эталон §2 снимался через
# `docker exec` в живом контейнере, а здесь идёт `compose run`, который
# печатает баннер entrypoint'а («fancai Backend — Starting», «Workers»,
# «Log level») и предупреждение Docker про orphan-контейнеры. Прямой diff
# из-за этого НИКОГДА не будет пустым. Сравнивать надо содержательные
# строки alembic, а не весь вывод.
for f in check_pg17 check_pg18; do
  grep -E 'Detected|upgrade operations|WARNING|ERROR' /tmp/pg18/$f.txt \
    | sed -E 's/0x[0-9a-f]+/0xADDR/g' | sort > /tmp/pg18/$f.core
done
diff /tmp/pg18/check_pg17.core /tmp/pg18/check_pg18.core   # обязан быть пустым
```

Дальше:

- расширения совпадают с `ext_pg17.txt` из §2 ПЛЮС ожидаемое обновление
  `vector` до `0.8.6` (см. §5: паритет невозможен, образ 18 несёт только
  этот скрипт). Прежняя редакция требовала просто `vector → 0.8.6`, что
  верно для dev и неверно для прода, где до переезда было `0.8.2`;
- счётчики совпадают с `counts_pg17.txt`;
- smoke пайплайна зелёный против 18:

  ```bash
  # `GOOGLE_APPLICATION_CREDENTIALS` гасить ОБЯЗАТЕЛЬНО: в проде он задан
  # (Vertex ADC), и скрипт отказывается работать, чтобы прогон не стал
  # платным. В репетиции на dev переменной не было, поэтому в первой
  # редакции её здесь не было тоже.
  docker compose -f docker-compose.prod.yml run --rm --no-deps \
    -e OPENROUTER_API_KEY= -e GEMINI_API_KEY= -e GOOGLE_APPLICATION_CREDENTIALS= \
    backend python scripts/smoke_llm_book_pipeline.py all
  ```

  Ожидается шесть режимов `PASS`, ноль `FAIL`. Smoke создаёт временную
  книгу в БОЕВОЙ базе (`books` на время прогона +1) и убирает её сам —
  счётчики надо сверять ПОСЛЕ его завершения, иначе они не сойдутся.
