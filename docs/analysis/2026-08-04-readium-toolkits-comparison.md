...
|---|---|---|
| §3.3: swift-toolkit требует «Swift 6.0» | `Support/CocoaPods/ReadiumShared.podspec` → `s.swift_version = '5.10'` | низкая, но требование ниже заявленного |
...
- **B-последовательный** — все десять осей без дефицита и прямые релизы upstream, ценой собственного нативного слоя на каждой платформе (верхняя граница 8 097 + 12 624, фактически заметно меньше — см. §11) и второй реализации бизнес-логики, **и ценой того, что веб-читалка живёт примерно вдвое дольше** («Конфликт решений» §8.0).
