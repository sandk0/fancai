...
export function notifyFallbackOnce(): void {
  ...
  import('@/stores/ui').then(({ notify }) => {
    notify.warning('Хранилище недоступно', 'Данные не будут сохранены между сессиями');
  });
}
 ...
 */
export function createChapterId(userId: string, bookId: string, chapterNumber: number): string {
 ...
 */
export function parseOfflineBookId(id: string): { userId: string; bookId: string } | null {
