 ...
 * 2. IndexedDB "FancaiCache" - epub locations cache (via clearCachedLocations)
...
import { bookKeys, chapterKeys, descriptionKeys, imageKeys, pwaKeys } from '@/hooks/api/queryKeys';
 ...
 * ```
 */
export async function resetBookData(userId: string, bookId: string): Promise<BookDataResetResult> {
  ...
  // 2. Clear IndexedDB "FancaiCache" (epub locations)
  try {
