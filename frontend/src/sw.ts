...
registerRoute(({ url }) => url.pathname.includes('/parsing-status'), new NetworkOnly());
...
registerRoute(
  ({ url, request }) =>
    ...
    // Exclude user-specific data (privacy: cleared on logout)
...
registerRoute(
  ({ request, url, sameOrigin }) => request.destination === 'image' && sameOrigin,
  new CacheFirst({
...
registerRoute(
  ({ request, url, sameOrigin }) =>
    request.destination === 'image' && !sameOrigin && !url.hostname.includes('pollinations.ai'),
  new StaleWhileRevalidate({
...
self.addEventListener('message', (event: ExtendableMessageEvent) => {
  ...
  if (event.data.type === 'LOGOUT') {
    console.log('[SW] Logout — clearing user-sensitive caches');
    event.waitUntil(
      Promise.all([caches.delete('api-cache'), caches.delete('generated-images-cache')]).then(
        () => {
          console.log('[SW] User-sensitive caches cleared');
        }
      )
    );
  }
 ...
 * Default notification options by type
 */
function getNotificationOptions(type: string, payload: PushPayload): ExtendedNotificationOptions {
        ...
        if ('focus' in client && 'navigate' in client) {
          return (client as WindowClient).focus().then((focusedClient) => {
            if (focusedClient && 'navigate' in focusedClient) {
              return (
                focusedClient as WindowClient & { navigate: (url: string) => Promise<WindowClient> }
              )
                .navigate(url)
