...
export const ParsingOverlay: React.FC<ParsingOverlayProps> = ({
  ...
  const {
    ...
    onError: (error) => {
      // WebSocket 'error' messages from server are parsing errors (type='error' in WS message)
      // Connection errors are handled by wsStatus changes and onerror/onclose events
      if (error && error !== 'Processing timeout: no progress received') {
        logger.warn('[ParsingOverlay] Parsing error from WebSocket:', error);
        setParsingError(error);
      } else {
        logger.warn(
          '[ParsingOverlay] WebSocket timeout/connection error, falling back to polling:',
          error
        );
    ...
    const checkProgress = async () => {
      ...
      try {
        ...
        if (statusData.status === 'error' || statusData.status === 'failed') {
          setParsingError(
            statusData.parsing_error || statusData.error || t('errors.parsing_failed')
          );
          return; // Stop polling
        }
  ...
  const handleRetryParsing = useCallback(async () => {
    try {
      ...
      referenceProgressRef.current = null;
      await booksAPI.processBook(bookId);
      notify.success(t('errors.parsing_retry_started'));
    } catch (err) {
      logger.error('[ParsingOverlay] Retry failed:', err);
      notify.error(t('errors.parsing_failed'));
      setParsingError(t('errors.parsing_failed'));
    }
  }, [bookId, t]);
        ...
        <div className="flex flex-col items-center gap-4 px-6 text-center">
          ...
          <div className="flex gap-3">
            ...
            {onCancel && (
              <button
                onClick={onCancel}
                className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-white/70 transition-all"
              >
                {t('ui.parsing.cancel')}
              </button>
            )}
