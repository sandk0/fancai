...
import { isAndroid, isIOS } from '@/utils/iosSupport';
import { logger } from '@/lib/logger';

// Android selection handles (carets) extend below the selection rect by ~24px
const ANDROID_CARET_OFFSET = 24;
// iOS selection handles extend below text by ~12px
const IOS_CARET_OFFSET = 12;
  ...
  useEffect(() => {
    ...
    const handleSelected = (cfiRange: string, contents: Contents) => {
      try {
        ...
        const caretOffset = isAndroid() ? ANDROID_CARET_OFFSET : isIOS() ? IOS_CARET_OFFSET : 0;
