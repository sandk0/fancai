...
class MergeGroup(BaseModel):
    ...
    duplicate_ids: List[str] = Field(description="ID сущностей-дубликатов для объединения")
    confidence: float = Field(description="Уверенность в том, что это дубликаты (0.0-1.0)")
...
class EntityDeduplicationService:
    ...
    async def suggest_merges(self, book_id: UUID) -> DeduplicationResponse:
        ...
        try:
            ...
            logger.info(f"[EntityDedup] Found {len(response.merge_groups)} merge groups")
            return response
    ...
    def _prepare_entities(self, entities: List[Entity]) -> List[EntityForAnalysis]:
        ...
        for e in entities:
            ...
            result.append(EntityForAnalysis(
                id=str(e.id),
                name=e.name,
                type=str(e.type),
