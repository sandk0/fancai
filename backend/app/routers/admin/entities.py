...
async def get_duplicate_entities(
    ...
    logger.info(
        f"[AdminEntities] Found {len(duplicate_groups)} duplicate groups, {total_duplicates} extra entities"
    )

    return DuplicatesResponse(
        groups=duplicate_groups, total_duplicates=total_duplicates
    )
...
async def _merge_entities_internal(
    ...
    duplicates_result = await db.execute(
        select(Entity).where(Entity.id.in_(duplicate_ids))
    )
    ...
    existing_desc_ids_result = await db.execute(
        select(DescriptionEntity.description_id).where(
            DescriptionEntity.entity_id == master_id
        )
    )
    ...
    if existing_desc_ids:
        await db.execute(
            delete(DescriptionEntity).where(
...
async def merge_entities(
    ...
    if master_id in duplicate_ids:
        raise HTTPException(
            status_code=400, detail="Master ID cannot be in duplicate IDs"
        )

    logger.info(
        f"[AdminEntities] Merging {len(duplicate_ids)} entities into master {master_id}"
    )
...
async def update_mention_cfi(
    ...
    stmt = (
        update(EntityMention)
        .where(EntityMention.id == request.mention_id)
        .values(mention_cfi=request.cfi)
    )
...
async def update_entity_first_mention_cfi(
    ...
    update_stmt = (
        update(EntityMention)
        .where(EntityMention.id == mention_id)
        .values(mention_cfi=request.cfi)
    )
    ...
    entity_result = await db.execute(
        select(Entity.book_id).where(Entity.id == request.entity_id)
    )
