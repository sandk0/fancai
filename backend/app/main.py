...
if __name__ == "__main__":
    ...
    #
    # Директива подавления обязана стоять на строке САМОЙ находки. Раньше она
    # висела на `log_level`, а B104 срабатывает на `host`, поэтому не
    # действовала («Total lines skipped: 0») и джоба `Backend SAST (Bandit)`
    # падала на единственной Medium-находке.
    #
    # Привязка к 0.0.0.0 здесь безопасна: этот блок исполняется только при
    # `python app/main.py` на машине разработчика, а в контейнерах команда
    # запуска своя (uvicorn/gunicorn) и слушать все интерфейсы внутри
    # контейнера — норма, наружу порт открывает compose.
    uvicorn.run(
        "main:app",
