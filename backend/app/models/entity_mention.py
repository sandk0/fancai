...
class EntityMention(Base):
    ...
    chapter_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("chapters.id", ondelete="CASCADE"), nullable=False, index=True
    )
    ...
    # Контекст упоминания
    mention_text: Mapped[str] = mapped_column(String(500), nullable=False)  # Текст, который нашли (например "Геральт" или "Белый Волк")
    context: Mapped[str | None] = mapped_column(Text, nullable=True)  # Окружающий текст для контекста
