...
def cleanup_old_images_task(days_old: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            "Image cleanup completed", deleted_records=result.get("deleted_records")
        )
        return result
    ...
    async with AsyncSessionLocal() as db:
        ...
        for image in old_images:
            try:
                ...
                logger.error(
                    "Error deleting image", image_id=str(image.id), error=str(e)
                )
                continue
...
async def _cleanup_stuck_books_async() -> Dict[str, Any]:
    ...
    async with AsyncSessionLocal() as db:
        ...
        query = select(Book).where(
            Book.is_processing.is_(True), Book.updated_at < cutoff
        )
        ...
        for book in stuck_books:
            ...
            book.descriptions_processing_error = f"Cleaned by scheduled task: stuck for 4+ hours (detected at {datetime.now(timezone.utc).isoformat()})"
            ...
            try:
                ...
                logger.warning(
                    f"Failed to invalidate cache for book {book.id}: {cache_e}"
                )
