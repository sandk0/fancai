...
- **Панели читалки были недостижимы в Safari** (2026-08-07). Найдено
  ...
  **Проверено владельцем в живом Safari 26.5.2**, а не только в патченной
  ...
  Escape достаётся ровно одному владельцу: читалка молчит, пока открыт
  любой оверлей со своим слушателем — панели, `SelectionMenu` при живом
  выделении ИЛИ в режиме правки заметки, модалка картинки (через `enabled`).
  Флаг называется `escapeHandledByOverlay`, а не `isPanelOpen`, потому что
  меню выделения панелью не является: оно в `isPanelOpen` не входило, и
  двойной эффект (закрыть меню И раскрыть шапку) воспроизводился в Chromium,
  Firefox и Mobile Chrome — там, где выделение работает. Под тестами
  и сам хук, и сборка флага: сужение до `isPanelOpen` компилируется молча.
