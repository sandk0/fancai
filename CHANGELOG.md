...
- **Откат non-critical пост-фазы ломал обработку книги двумя способами**
  ...
  1. **Финализация падала `MissingGreenlet`** — `graph_service.calculate_pagerank()`
     при исключении делает `await self.db.rollback()` (а он срабатывает на каждой
     книге: `nx.pagerank` требует отсутствующий `scipy`), после чего обращение
     к `book.id` в `_finalize_book_status()` уходило в синхронный lazy-load.
     Книга оставалась с `is_processing=true`, celery-задача завершалась ошибкой.
     Теперь книга перечитывается через `db.get()` перед финализацией.
  ...
  Дополнительно: упавший auto-merge (`_merge_entities_internal`, вызывается
  из таска мимо HTTP-обёртки и своего отката не делает) больше не оставляет
  сессию в failed-состоянии — добавлен `rollback()` в обработчик.

  Дефекты воспроизведены и на коде до удаления GLiNER. Регрессию ловит
  `scripts/smoke_llm_book_pipeline.py failing`.
