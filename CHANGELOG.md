...
- **Выкатка 2026-08-07:** прод переведён с `b8f26714` на `7f95942a`.
  Миграций нет, данные целы (3 / 16 / 554 / 802 / 950 / 46 / 16 до и после),
  SPA совпал с образом побайтово. Что на проде именно новый код — проверено
  поиском `reader-ready` в опубликованном `assets/js/ReaderPage-*.js`,
  а не только совпадением файлов.

  Две грабли на будущее. `git push` перед выкаткой обязателен: `update_code`
  делает `merge --ff-only origin/main`, поэтому без публикации скрипт
  отчитался бы успехом и выкатил предыдущий коммит. И SSH обрывается
  на ~60-й секунде второй раз подряд — длинные шаги только через
  `setsid nohup`, `tmux` на сервере нет.

- **PostgreSQL 17.10 → 18.4** (2026-08-07). Мажорный апгрейд не переживает
  смену тега: каталог данных 17 сервером 18 не читается, поэтому путь —
  `pg_dump` → новый том → `pg_restore`. Простой ~12 минут (очереди Celery
  были пусты, дренажа не потребовалось). Счётчики до и после совпали,
  `alembic heads` == `current` == `g1h2i3j4k5l6`, дрейф `alembic check`
  совпал с эталоном pg17, шесть режимов smoke зелёные. Старый том
  `app_postgres_data` сохранён точкой отката.

  Правок в `docker-compose.prod.yml` шесть, а не одна: тег сервера, удаление
  явного `PGDATA`, путь монтирования без `/data`, `name: app_postgres_data_pg18`,
  тег sidecar'а бэкапа и **снятие монтирования `./postgres/init`**. Последнее
  — блокер, найденный до окна: скрипты оттуда идут только при пустом PGDATA,
  то есть на новом томе впервые за всю жизнь прода, а `01-extensions.sql`
  делает `\c bookreader` при базе `fancai` и с `ON_ERROR_STOP=1` сорвал бы
  инициализацию.

  Неизбежное следствие: `vector` 0.8.2 → 0.8.6. Пин старой версии невозможен
  — образ 18 несёт один установочный скрипт своей версии, путей вниз
  у pgvector нет.
...
- **Панели читалки были недостижимы в Safari** (2026-08-07). Найдено
  ...
  **Проверено владельцем в живом Safari 26.5.2**, а не только в патченной
  ...
  куда sandbox не достаёт. **Правка проверена в живом Safari 26.5.2:
  панели раскрываются.** Листание кликом внутри книги там по-прежнему
  невозможно — это ограничение движка.

  Escape достаётся ровно одному владельцу, и для этого нужны ДВА рубежа.
  Флаг `escapeHandledByOverlay` (панели, живое выделение, правка заметки;
  модалка картинки — через `enabled`) закрывает случай, когда нажатие
  вообще не доходит до родительских слушателей. Его одного оказалось
  мало: слушатель читалки висит на `window`, то есть позже слушателя
  `SelectionMenu` на `document`, а React флашит keydown синхронно —
  меню успевает закрыться, сбросить `selection` и заставить эффект
  пересоздать слушатель с уже пустым флагом. Поэтому `SelectionMenu`
  помечает нажатие `preventDefault()`, а читалка пропускает
  израсходованные события. Дефект виден только в Chromium, Firefox
  и Mobile Chrome: на WebKit меню выделения не всплывает вовсе.

  Под тестами оба рубежа: 6 на хук, 2 на сборку флага в `EpubReader`
  и один e2e с настоящим `SelectionMenu` — последний и поймал дефект
  порядка, потому что юнит-тесты меню мокают.
